...
def get_tag_detail(tag_id: int, db: Session = Depends(get_db)) -> TagRead:
...
def post_profile(payload: CustomerProfileCreate, db: Session = Depends(get_db)) -> CustomerProfileRead:
    return create_customer_profile(db, payload)


@router.get("/profiles", response_model=list[CustomerProfileRead])
def get_profiles(db: Session = Depends(get_db)) -> list[CustomerProfileRead]:
    return list_customer_profiles(db)


@router.get("/profiles/{customer_id}", response_model=CustomerProfileRead)
def get_profile_detail(customer_id: int, db: Session = Depends(get_db)) -> CustomerProfileRead:
    return get_customer_profile(db, customer_id)


@router.patch("/profiles/{customer_id}", response_model=CustomerProfileRead)
def patch_profile(customer_id: int, payload: CustomerProfileUpdate, db: Session = Depends(get_db)) -> CustomerProfileRead:
...
def remove_profile(customer_id: int, db: Session = Depends(get_db)) -> Response:
...
def post_blacklist(payload: BlacklistCreate, db: Session = Depends(get_db)) -> BlacklistRead:
    return create_blacklist_entry(db, payload)


@router.get("/blacklist", response_model=BlacklistListResponse)
def get_blacklist(db: Session = Depends(get_db)) -> BlacklistListResponse:
...
def get_blacklist_detail(blacklist_id: int, db: Session = Depends(get_db)) -> BlacklistRead:
    return get_blacklist_entry(db, blacklist_id)


@router.patch("/blacklist/{blacklist_id}", response_model=BlacklistRead)
def patch_blacklist(blacklist_id: int, payload: BlacklistUpdate, db: Session = Depends(get_db)) -> BlacklistRead:
...
def remove_blacklist(blacklist_id: int, db: Session = Depends(get_db)) -> Response:
