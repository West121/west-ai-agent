...
def post_document(payload: KnowledgeDocumentCreate, db: Session = Depends(get_db)) -> KnowledgeDocumentRead:
...
def get_documents(db: Session = Depends(get_db)) -> list[KnowledgeDocumentRead]:
    return list_knowledge_documents(db)


@router.get("/documents/{document_id}", response_model=KnowledgeDocumentRead)
def get_document_detail(document_id: int, db: Session = Depends(get_db)) -> KnowledgeDocumentRead:
...
def post_submit_review(document_id: int, db: Session = Depends(get_db)) -> KnowledgeDocumentRead:
...
def post_rebuild_index(document_id: int, db: Session = Depends(get_db)) -> KnowledgeIndexTaskResultRead:
